...
// 存到 xhfly.userfreq,重启后加载,真正反映用户的输入习惯。
...
int main(int argc, char *argv[]) {
  ...
  if (len > 0) {
    ...
    if (slash) {
      strcpy(slash + 1, "xhfly.dict");
      dict_ok = dict_load(dict_path);
      strcpy(slash + 1, "xhfly.userfreq");
  ...
  if (!dict_ok)
    dict_ok = dict_load("xhfly.dict");
  if (!dict_ok && argc > 1)
    dict_ok = dict_load(argv[1]);

  if (!dict_ok) {
    fprintf(stderr, "cannot load xhfly.dict\n");
    return 1;
  }
  ...
  while (s.running) {
    ...
    // 用户词频定时落盘:内存中即时累加,周期性批量落盘。
    // 间隔 3 分钟,平时尽量少写盘;退出前 / SIGTERM/SIGINT 时强制 flush。
    if (G_USERFREQ_DIRTY) {
